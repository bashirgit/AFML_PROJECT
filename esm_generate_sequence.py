...
HF_TOKEN = "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"
...
def generate_sequence(model, tokenizer, seq_length, num_iterations,
    ...
    tokens_per_sec = seq_length / total_time if total_time > 0 else 0

    return decoded_seq, total_time, memory_used, avg_iter_latency, tokens_per_sec
...
csv_header = [
    'seq_id', 'sequence_length', 'generation_time_sec', 'throughput_tokens_per_sec',
    'tokens_per_sec', 'memory_usage_mb', 'perplexity', 'seed',
...
with open(FASTA_FILE, 'w') as fasta_out, open(LOG_FILE, 'w', newline='') as csv_out:
    ...
    for i in tqdm(range(NUM_SEQUENCES), desc="Generating sequences"):
        ...
        sequence, total_time, memory_used, avg_iter_latency, tokens_per_sec = generate_sequence(
        ...
        csv_writer.writerow({
            ...
            'throughput_tokens_per_sec': round(avg_iter_latency, 6),
